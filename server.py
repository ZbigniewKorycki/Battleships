...
class Server:
    def __init__(self):
        ...
        self.comm_utils = CommunicationUtilsServer(self.ai_player)
        self.db_comm_utils = DatabaseCommunicationUtils()
    ...
    def create_response_to_client(self, client_request):
        if client_request["type"] == "GAME_INVITATION":
            response_for_game_invitation = (
                self.comm_utils.server_game_invitation_response()
            )
            ...
            return self.comm_utils.server_response_for_client_shot(client_request)
        elif client_request["type"] == "SHOT_REQUEST":
            return self.comm_utils.server_shot()
            ...
            return self.db_comm_utils.save_game_to_db()
        elif client_request["type"] == "GAME_NUMBER":
            return self.db_comm_utils.establish_game_number()
            ...
            return self.db_comm_utils.save_board_to_db(player_board, opponent_board)
        elif client_request["type"] == "SAVE_WINNER":
            winner = client_request["body"]["winner"]
            return self.db_comm_utils.set_winner_in_table(winner)
        elif client_request["type"] == "ARCHIVED_GAMES":
            return self.db_comm_utils.show_all_games()
        elif client_request["type"] == "WATCH_GAME":
            game_id = client_request["body"]["game_id"]
            return self.db_comm_utils.show_one_game(game_id)
        else:
            return self.comm_utils.server_response_to_unknown_command()

    def clean_up_database(self):
        self.db_comm_utils.delete_game_boards_without_winner("client_boards")
        self.db_comm_utils.delete_game_boards_without_winner("server_boards")
        self.db_comm_utils.delete_games_without_winner()
